    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 1000){
            if(resultCode == Activity.RESULT_OK){
                ...
                Glide.with(addProductFragment.this).load(imageUri).override(512, 512).centerCrop().into(mimgSP);
...
//        Glide.with(addFragment.this).load(imageUri).override(100, 100).centerCrop().into(mimgSP);
        if(imageUri != null){
            ...
            myRef.putFile(imageUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    ...
                    myRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            if (uri != null){
                                ...
                                Toast.makeText(addProductFragment.this.getActivity(),"Không tìm thấy uri",Toast.LENGTH_SHORT).show();
                            }
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(addProductFragment.this.getActivity(), "Lỗi lấy đường link ảnh", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(addProductFragment.this.getActivity(), "Lỗi tải ảnh lên Firebase Storage", Toast.LENGTH_SHORT).show();
                }
            });
        }
                    ...
                    myRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            if (uri != null){
                                imageURL = uri.toString();
                                if(imageURL == null){
                                    Toast.makeText(addProductFragment.this.getActivity(),"Chưa tìm thấy đường dẫn hình ảnh",Toast.LENGTH_SHORT).show();
                                }else {
...
//                    }
                                    collectionReference.document(id).set(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            Toast.makeText(addProductFragment.this.getActivity(),"Thêm thành công",Toast.LENGTH_SHORT).show();
                                            getParentFragmentManager().popBackStack();
                                        }
                                    }).addOnFailureListener(new OnFailureListener() {
                                        @Override
                                        public void onFailure(@NonNull Exception e) {
                                            Toast.makeText(addProductFragment.this.getActivity(),"Thêm thất bại",Toast.LENGTH_SHORT).show();
                                        }
                                    });
                                }
                            }
